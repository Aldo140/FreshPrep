  ...
  if (!computedMetrics) {
    return (
      <div 
        ...
        className="p-6 rounded-xl border border-dashed border-emerald-205 bg-white shadow-xs text-center flex flex-col items-center justify-center min-h-[160px]"
      >
        <Activity className="w-8 h-8 text-emerald-600 animate-pulse mb-2.5" />
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1 px-2 py-1 sm:px-3 sm:py-1.5 rounded-md text-[11px] sm:text-xs font-bold transition-all cursor-pointer ${
                isActive
                  ? "bg-white border border-slate-205/55 text-emerald-700 shadow-2xs"
                  : "text-slate-500 hover:text-slate-800 hover:bg-slate-100/50"
              }`}
            >
              <Icon className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${isActive ? "text-emerald-600" : "text-slate-400"}`} />
              {t.label}
            </button>
          );
        })}
      ...
      <div className="bg-slate-50/50 border border-slate-100 p-4 rounded-xl text-xs leading-relaxed text-slate-700 space-y-3 font-sans">
        ...
        {selectedTopic === "summary" && (
          <div className="space-y-3 animate-fade-in" id="topic-summary-panel">
            <div className="flex justify-between items-center bg-white border border-slate-200/50 p-2.5 rounded-lg">
              ...
              <span className="px-2 py-0.5 bg-emerald-50 text-emerald-800 border border-emerald-100 rounded text-[9px] font-bold uppercase tracking-wider font-mono">
                Stable Benchmark
              </span>
            </div>
            
            <p className="text-slate-600">
              The audited portfolio currently encompasses <strong className="text-slate-800">{totalMatchedCodes} active event codes</strong>. Blended performance returns are yielding a conversion baseline of <strong className="font-mono text-slate-800">{summary.blendedConversionRate.toFixed(1)}%</strong> across all associated marketing channels. This output represents a total customer cohort contribution value of <strong className="text-emerald-700 font-bold">${summary.totalLTV12.toLocaleString()}</strong> over a 12-month timeline.
            </p>
        ...
        {selectedTopic === "top" && topPerformerCode && (
          <div className="space-y-3 animate-fade-in" id="topic-top-panel">
            ...
            <p className="text-slate-600">
              Promo code <strong className="text-slate-950 font-mono">{topPerformerCode.discount_code}</strong> represents the highest overall operational efficiency. It yielded <strong className="text-slate-900 font-mono">{topPerformerCode.Signups}</strong> venue signups, maintaining a highly healthy subscription conversion index of <strong className="text-emerald-700 font-bold font-mono">{topPerformerCode.calculatedConversion.toFixed(1)}%</strong> and generating <strong className="text-emerald-700 font-bold">${topPerformerCode["Sum LTV 12"].toLocaleString()}</strong> in actual customer lifetime volume.
            </p>
          ...
          <div className="space-y-3 animate-fade-in" id="topic-highest-conv-panel">
            <div className="flex justify-between items-center bg-white border border-slate-200/50 p-2.5 rounded-lg">
              <div>
                ...
                <p className="text-base font-extrabold text-emerald-800 font-sans mt-0.5">{highestConvCode.discount_code}</p>
              </div>
              <span className="px-2 py-0.5 bg-emerald-50 text-emerald-800 border border-emerald-100 rounded text-[9px] font-bold uppercase tracking-wider font-mono">
            ...
            <p className="text-slate-600">
              Campaign checkout records verify <strong className="text-slate-950 font-mono">{highestConvCode.discount_code}</strong> achieved the maximum Conversion profile of <strong className="text-emerald-700 font-bold font-mono">{highestConvCode.calculatedConversion.toFixed(1)}%</strong>. This code registered <strong className="text-slate-800 font-mono">{highestConvCode["Paying cx"]}</strong> subscription conversions out of <strong className="text-slate-800 font-mono">{highestConvCode.Signups}</strong> total signups, and returned <strong className="text-slate-800">${highestConvCode["Avg LTV 12"].toFixed(0)}</strong> in average customer value.
            </p>
          ...
          <div className="space-y-3 animate-fade-in" id="topic-highest-ltv-panel">
            <div className="flex justify-between items-center bg-white border border-slate-200/50 p-2.5 rounded-lg">
              <div>
                ...
                <p className="text-base font-extrabold text-amber-800 font-sans mt-0.5">{highestLtvCode.discount_code}</p>
              </div>
              <span className="px-2 py-0.5 bg-amber-50 text-amber-800 border border-amber-100 rounded text-[9px] font-bold uppercase tracking-wider font-mono">
          ...
          <div className="space-y-3 animate-fade-in" id="topic-lowest-panel">
            <div className="flex justify-between items-center bg-white border border-slate-200/50 p-2.5 rounded-lg">
              <div>
                ...
                <p className="text-base font-extrabold text-rose-800 font-sans mt-0.5">{lowestPerformerCode.discount_code}</p>
              </div>
              <span className="px-2 py-0.5 bg-rose-50 text-rose-800 border border-rose-100 rounded text-[9px] font-bold uppercase tracking-wider font-mono">
                {lowestPerformerCode.calculatedConversion.toFixed(1)}% Conversion
              </span>
            </div>

            <p className="text-slate-600">
              Promo code <strong className="text-slate-900 font-mono">{lowestPerformerCode.discount_code}</strong> currently ranks as the lowest converting segment at <strong className="text-rose-600 font-bold font-mono">{lowestPerformerCode.calculatedConversion.toFixed(1)}%</strong>. Active signups stands at <strong className="font-mono text-slate-800">{lowestPerformerCode.Signups}</strong> resulting in <strong className="font-mono text-slate-800">{lowestPerformerCode["Paying cx"]}</strong> subscription buyers.
            </p>
